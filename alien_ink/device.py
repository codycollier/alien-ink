...
Target machine: Mist (local RTX 3070, ~8 GB). CUDA is preferred; MPS and CPU
are fallbacks. No TPU / XLA support.
"""
...
def distributed_world_size() -> int:
    """Process group size from ``WORLD_SIZE``, else 1."""
    try:
...
def resolve_precision(
    ...
    *,
    ...
    loss-scaling / overflow instabilities that fp16 can hit. The Mist RTX 3070
    (Ampere) supports bf16. MPS stays on fp32.
    """
    if device == "cuda":
...
class AcceleratorInfo:
    """Hardware / runtime fingerprint for run artifacts."""
...
# Approximate peak TFLOPS for MFU estimates. Mist baseline: RTX 3070 = 40.6 FP16.
...
def introspect(
    *,
    ...
    """Return a bordered runtime summary for verification.
