#!/usr/bin/env python
"""Pretrain Pythia-31M to a 20:1 token budget, ending on WikiText-103.

The 18,921-step budget is the Chinchilla planning approximation for 31M
parameters at 32,768 tokens per optimizer step. Training streams English
Wikipedia for 15,245 steps, then makes one complete packed pass through
WikiText-103 in 3,676 full optimizer steps.

  python alien_ink/zdeck/pre_pythia-31m_wikipedia_wikitext_chinchilla_mist.py
"""
...
CURRICULUM = Curriculum(
    phases=(
        CurriculumPhase(data=WIKIPEDIA_DATA, steps=15_245, label="wikipedia"),
...
MANIFEST = Manifest(
    run_name="pre-pythia-31m-wikipedia-wikitext-chinchilla-mist",
    title="Pythia-31M Chinchilla pretraining: Wikipedia then WikiText-103",
    stage="pre",
    data=CURRICULUM,
    ...
    wandb=WandbConfig(
        entity="logbook",
        project="ink-explore",
        name="pre-pythia-31m-wikipedia-wikitext-chinchilla-mist",
        enabled=True,
    ),
