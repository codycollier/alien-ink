#!/usr/bin/env python
"""Short tinygrad GPT-2 performance baseline on fully materialized WikiText-103.

Same knobs as ``baseline_perf_gpt-2_mist.py`` (HF Trainer). Compare side by side:

  ./bin/mist-train-baseline-perf.sh gpt-2
  ./bin/mist-train-baseline-perf.sh gpt-2-tinygrad

  python alien_ink/zdeck/baseline_perf_gpt-2_tinygrad_mist.py
"""
...
RUN_NAME = "baseline-perf-gpt-2-tinygrad-mist"

MANIFEST = Manifest(
    run_name=RUN_NAME,
    title="Tinygrad GPT-2 from scratch on WikiText-103 (0.25 epochs, baseline perf)",
    stage="pre",
...
if __name__ == "__main__":
    from alien_ink.tg.pretrain import pretrain
