...
def test_resolve_device_prefers_cuda(monkeypatch):
    monkeypatch.setattr(device_mod.torch.cuda, "is_available", lambda: True)
...
def test_introspect_cuda(monkeypatch):
    ...
    text = device_mod.introspect()
    assert device_mod._kv("torch", "2.2.0+cu121") in text
    assert device_mod._kv("cuda", "True  (12.1, cudnn 8902)") in text
    assert device_mod._kv("gpu", "NVIDIA GeForce RTX 3070 (8 GB)") in text
    ...
    assert device_mod._kv("peak_tflops", "40.6") in text
