...
Track B0 of the [model learning plan](model-learning-plan.md) is implemented,
...
[gpu-memory.md](gpu-memory.md): `P × 12–16` bytes for bf16 weights + grads +
...
available on the Hub as of mid-2026.
...
| [`HuggingFaceTB/SmolLM2-360M`](https://huggingface.co/HuggingFaceTB/SmolLM2-360M) | 360M | Same block, 32 layers | ~49k | 8k | Apache-2.0 | First scale step; strongest ≤400M English base |
...
| [`Qwen/Qwen3-0.6B-Base`](https://huggingface.co/Qwen/Qwen3-0.6B-Base) | 0.6B | 28 layers, GQA; transformers ≥ 4.51 | ~152k | 32k | Apache-2.0 | The ceiling; benchmark-consensus best tunability under 1B |
| [`LiquidAI/LFM2-350M`](https://huggingface.co/LiquidAI/LFM2-350M) | 350M | Hybrid conv + attention | ~65k | 32k | LFM Open License | Optional curiosity — tops 2025/26 tunability benchmarks; nonstandard arch, verify `AutoModel` path first |

Beyond the line — `Llama-3.2-1B` (1.23B, 128k vocab), `OLMo-2-1B`,
`SmolLM2-1.7B` — full Adam state alone exceeds the card. They stay in the
[large-base roadmap](roadmap-large-base-mist.md) where QLoRA pays their rent.

Scouting rules that generalize past this table:

- prefer **suites** (Pythia, SmolLM2, Qwen) — size ablations inside one
  family isolate scale from architecture;
